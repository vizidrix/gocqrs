...
type ApplyCommand func(idgen AggregateIdGenerater, aggregate Aggregate, command Command) (Event, error)
...
func (c *commandHandler) hydrateAggregate(events []Event) (Aggregate, error) {
	return c.hydrator(events)
}

func (c *commandHandler) applyCommand(aggregate Aggregate, command Command) (Event, error) {
	return c.applicator(c.idGen, aggregate, command)
}

func (c *commandHandler) publishEvent(event Event) error {
	return c.publisher(event)
}
...
func (c *commandHandler) Handle(command Command) error {
	...
	aggregate, err := c.hydrateAggregate(events)
	if err != nil {
		return ErrUnableToLoadAggregate
	}
	// Evaluate the command against the aggregate
	event, err := c.applyCommand(aggregate, command)
	if err != nil {
